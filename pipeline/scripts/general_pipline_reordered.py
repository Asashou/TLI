...
def txt2dict(path):
    ...
    if 'all' in input_txt['steps']:
        input_txt['steps'] = ['preshift', 'postshift', 'ants', 'mask', 'n2v', 'clahe']
...
def main():
    ...
    for round, ind in enumerate(scope):
        ...
        if 'n2v' in input_txt['steps']:
            ...
            image[input_txt['ch_names'][0]] = N2V_predict(image=image[input_txt['ch_names'][0]],
        ...
        if 'clahe' in input_txt['steps']:
            ...
            image[input_txt['ch_names'][0]] = apply_clahe(kernel_size=input_txt['kernel_size'], 
                        image=image[input_txt['ch_names'][0]], clipLimit=input_txt['clipLimit'], 
                        xy_pixel=input_txt['xy_pixel'], z_pixel=input_txt['z_pixel'],
                        save=True, save_path=input_txt['save_path'], 
                        save_file=input_txt['ch_names'][0]+'_'+file)        
        if 'mask' in input_txt['steps']:
            image[input_txt['ch_names'][0]] = mask_subset(image[input_txt['ch_names'][0]], sig=6,
                                                                save=True, save_path=input_txt['save_path'],
                                                                xy_pixel=input_txt['xy_pixel'], z_pixel=input_txt['z_pixel'],
                                                                save_file=input_txt['ch_names'][-1]+'_'+file)
        if 'ants' in input_txt['steps']:
            # reg_img = image.copy()
            # if len(input_txt['ch_names']) > 1:
            #     # reg_img[input_txt['ch_names'][-1]] = N2V_predict(image=reg_img[input_txt['ch_names'][-1]],
            #     #                                                 model_name=input_txt['model_name'], 
            #     #                                                 model_path=input_txt['model_path'], 
            #     #                                                 save=True, save_path=input_txt['save_path'],
            #     #                                                 xy_pixel=input_txt['xy_pixel'], z_pixel=input_txt['z_pixel'],
            #     #                                                 save_file=input_txt['ch_names'][-1]+'_'+file) 
            #     if 'mask' in input_txt['steps']:
            #         image[input_txt['ch_names'][0]] = mask_subset(image[input_txt['ch_names'][0]], sig=12,
            #                                                       save=True, save_path=input_txt['save_path'],
            #                                                       xy_pixel=input_txt['xy_pixel'], z_pixel=input_txt['z_pixel'],
            #                                                       save_file=input_txt['ch_names'][-1]+'_'+file)
            if round == 0:
                ref = image.copy()
                start_ref = image.copy()
        ...
        if 'clahe' in input_txt['steps']:
            print('applying clahe on', file)
            apply_clahe(kernel_size=input_txt['kernel_size'], 
                        image=image[input_txt['ch_names'][0]], clipLimit=input_txt['clipLimit'], 
                        xy_pixel=input_txt['xy_pixel'], z_pixel=input_txt['z_pixel'],
                        save=True, save_path=input_txt['save_path'], 
                        save_file=input_txt['ch_names'][0]+'_'+file)
